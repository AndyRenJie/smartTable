    ...
    public class ExcelAsyncTask<K> extends AsyncTask<Integer, Void, K[][]> {
        ...
        protected void onPostExecute(K[][] data) {
            if (callback != null && softReference.get() != null) {
                ...
                //设置列标题，因为加载Excel文件把数据当成单元格内容没有设置列标题，固定列标题属性无效
                //所以拿到数据后设置一下列标题，并把原来单元格的数据清空
                List<String> titleNames = new ArrayList<>();
                Cell[][] cellArray = (Cell[][]) data;
                for (int i = 0; i < cellArray.length; i++) {
                    Cell cell = cellArray[i][0];
                    titleNames.add(cell.getContents());
                    cellArray[i][0] = null;
                }
                //设置字体
                ArrayTableData<K> tableData = ArrayTableData.create(smartTable, "", titleNames, data, new TextDrawFormat<K>() {
