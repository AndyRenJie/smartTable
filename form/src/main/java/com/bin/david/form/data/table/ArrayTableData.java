...
public class ArrayTableData<T> extends TableData<T> {
     ...
     * @return 创建的二维数组表格数据
     */
    public static <T> ArrayTableData<T> create(SmartTable table, String tableName, List<String> titleNames, T[][] data, IDrawFormat<T> drawFormat) {
        table.getConfig().setShowColumnTitle(true);
        List<Column<T>> columns = new ArrayList<>();
        for (int i = 0; i < data.length; i++) {
            T[] dataArray = data[i];
            Column<T> column = new Column<>(titleNames == null ? "" : titleNames.get(i), null, drawFormat);
    ...
    public static <T> ArrayTableData<T> create(SmartTable table, String tableName, T[][] data, IDrawFormat<T> drawFormat) {
        ...
        return create(table, tableName, null, data, drawFormat);
    }
